...
const SECTION_SUBTEXT = {
  Contact: 'People we have a relationship with that we want to reach out to on a cadence.',
...
function buildContactRow(c, todayStr) {
  ...
  return `
    <li class="detail-item ${overdue ? 'overdue' : ''}">
      ...
      ${c.clientPartner ? `<div class="detail-item-meta">Partner: ${escapeHtml(c.clientPartner)}</div>` : ''}
